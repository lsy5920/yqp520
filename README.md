...
- 立派金典、门规与禁律、宗门日常用于展示门派世界观、相处底线和江湖日常。
...
5. 勾选名册盟约后提交。
6. 提交成功会生成待审玉佩帖，可保存图片并分享给执事扫码审核。
...
5. 保存为“已入册”时会自动生成不含数字 `4` 的入册编号，并立即进入前台名册和人数统计；如果暂时不想展示，请先改为“暂缓”或“退回”。
...
2. 云栖海报页可以生成官方宣传海报。
3. 江湖名帖页可以生成个人名帖海报。
4. 云栖名册登记成功后可以生成待审玉佩帖，审核通过后详情页可以生成入册玉佩帖。
...
脚本会把历史上 `status = approved` 但公开开关异常的记录补为公开。新版前台只以 `status = approved` 判断是否进入名册，避免旧公开开关脏数据继续漏掉人数。
...
2026-04-26 08:08 【优化】恢复云栖名册手机端云层、玉佩漂浮、光晕模糊和卷轴展开动效，改为修正手机断点下筛选栏粘性布局与卷轴视口高度，保留美观同时减少闪屏。
2026-04-26 08:13 【修复】修复云栖名册手机端卷轴被顶部导航遮挡的问题，将卷轴浮层提升为全屏弹窗并补足顶部按钮空间。
2026-04-26 08:20 【优化】优化云栖名册性别展示，公开玉佩正面不再显示男生女生文字，只通过光效区分；登记表单性别光效只保留男生和女生两个选项。
2026-04-26 08:46 【优化】统一门派信物为云栖玉佩，替换旧版页面名称、旧版名册海报名和旧版江湖海报名，重写问心题库为立派金典口径，并同步更新海报、登记、审核入口和项目文档。
2026-04-26 09:03 【优化】将立派金典、问心题库、首页亮点、页脚结语和名册登记文案调回江湖门派语气，保留玉佩信物设定且不在站内标注文风说明。
2026-04-26 13:31 【优化】恢复网站《云栖派立派金典》正文与问心题库口径，统一站内标题为立派金典，继续保留云栖玉佩信物设定，并同步更新页面入口和项目文档。
2026-04-26 13:46 【修复】修复审核台新名帖保存为已入册后未自动公开的问题，前台名册和人数统计会正常包含新入册同门，并在数据库脚本中补齐历史已入册未公开记录。
2026-04-26 13:56 【修复】进一步修复名册页漏显已入册名单的问题，前台公开查询和数据库公开策略改为以已入册状态为准，不再被历史公开开关脏数据挡住。
