...
- 公开名册已经重构为“玉佩云海”：清白玉玉佩随机漂浮，玉佩正面展示道名、编号和身份；男生泛青蓝光，女生泛粉红光，未选择性别不泛光；点击玉佩会放大并展开卷轴，展示全部公开信息。
...
- `dao_name`：新增独立道名字段，必须以“云”开头，长度为 `2` 到 `3` 个字。
- `entry_generation`：新增入册辈分字字段，默认值为 `云`，后台可以修改。
...
- 已登记名帖会按性别自动补发云字道名；已入册名帖会按现有入册顺序重排编号，格式为 `云栖-云-001`，不再跳过含 `4` 的数字。
...
- 已登记记录的新表 `dao_name` 应全部以“云”开头。
- 已入册记录的新编号应从 `云栖-云-001` 开始连续排列，包含 `云栖-云-004`。
...
5. 卷轴中会展示江湖名、公开故事、地域、羁绊状态、玉佩光色、入册时间和标签。
...
3. 按五步填写江湖名、道名、真实姓名、性别、身份、地域、宣言、故事、标签、羁绊、封面和联系方式。
4. 道名必须以“云”开头，长度为 `2` 到 `3` 个字；江湖名会单独保留，不会被道名覆盖。
5. 性别选择会影响公开名册玉佩光效：男生青蓝光，女生粉红光。
6. 勾选名册盟约后提交。
7. 提交成功会生成待审玉佩帖，可保存图片并分享给执事扫码审核。
...
4. 点击左侧名帖后，可编辑道名、江湖名、真实姓名、辈分字、入册序号、身份、性别、地域、宣言、故事、标签、羁绊、封面、状态、公开开关、联系方式、审核备注和内部备注。
5. 保存为“已入册”时会自动生成连续入册编号，显示格式为 `云栖-云-001`；如果暂时不想展示，请先改为“暂缓”或“退回”。
...
本次版本新增了 `dao_name`、`entry_generation` 和 `gender_key` 字段，如果没有执行脚本，登记和后台保存都会失败。
...
入册编号必须是大于 `0` 的整数，现在允许包含数字 `4`。如果留空，审核通过时系统会自动生成，公开显示为 `云栖-云-001` 这种格式。

### 道名保存失败

道名必须以“云”开头，长度为 `2` 到 `3` 个字，例如 `云澈`、`云岚`、`云衡`。江湖名和道名是两个字段，后台可以分别修改。
...
2026-04-27 11:21 【优化】新增云字道名字段并保留原江湖名，登记表单和审核台支持单独编辑道名；数据库脚本会按性别为已登记名单补发云字道名，并按现有入册顺序重排编号为云栖-云-001格式，不再跳过含4编号。
