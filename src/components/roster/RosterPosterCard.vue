...
import { formatRosterDate, formatRosterEntryTitle, getRosterCoverGradient } from '@/utils/roster'
...
// 这里计算海报主标题，统一使用玉佩名帖作为新版视觉名称。
const posterTitle = computed<string>(() => (isPending.value ? '云海待审玉佩帖' : '云海入册玉佩帖'))
...
const entryNoText = computed<string>(() => (props.entry.entryNo ? formatRosterEntryTitle(props.entry.entryNo, props.entry.entryGeneration) : '待授入册编号'))

// 这里计算二维码说明，待审提醒保存分享，入册提醒查看公开详情。
const qrCaption = computed<string>(() => (isPending.value ? '保存此玉佩帖，分享给云司扫码审核' : '扫码查看公开入册详情'))
</script>
...
<template>
  ...
  >
    ...
    <section class="cloud-poster-card__body">
      <p>{{ entry.identityLabel }} 路 {{ entry.bondLabel }}</p>
      <h3>{{ entry.daoName }}</h3>
    ...
    <footer class="cloud-poster-card__footer">
      ...
      <div class="cloud-poster-card__qr">
        <img v-if="qrCodeUrl" :src="qrCodeUrl" :alt="qrHint || '云海玉佩帖二维码'" />
